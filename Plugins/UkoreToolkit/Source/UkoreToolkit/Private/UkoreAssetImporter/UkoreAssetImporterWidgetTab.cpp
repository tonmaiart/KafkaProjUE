...
{
	TArray<TSharedPtr<FShotData>> Selected = SUkoreAssetImporterWidgetTab::GetSelectedItemListView();
	...
	TSharedPtr<FShotData> SelectedTarget = Selected[0];
	...
	{
		if (SelectedTarget->FileType == "AlembicAnim") {
	...
	if (SelectedTarget->FileType == "Camera") {
		MenuBuilder.BeginSection("CameraSection", FText::FromString("Camera"));

		MenuBuilder.AddMenuEntry(
			FText::FromString("Reimport Camera Only"),
			FText::FromString("Update Camera to Camera Name \"RenderCam\" in Sequencer."),
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SUkoreAssetImporterWidgetTab::OnReimportCameraOnlyClicked))
		);
...
void SUkoreAssetImporterWidgetTab::OnBuildSequencerToSelectedShot()
{
	...
	UE_LOG(LogTemp, Log, TEXT("# Quick Build Sequencer"));

	// Get Selected Deploy from selection
	TArray<TSharedPtr<FShotData>> Selected =  SUkoreAssetImporterWidgetTab::GetSelectedItemListView();

	if (Selected.IsEmpty())
	{
		UE_LOG(LogTemp, Log, TEXT("Selected not found"));

		return;
	}


	TSharedPtr<FShotData>& TargetShotData = Selected[0];
	 

	// Get All Asset from selected
	FString& ShotName = TargetShotData->ShotName;
	FString CameraFilePath = TEXT("");
	TArray<TSharedPtr<FShotData>> AlembicAnimList;
	...
	for (TSharedPtr<FShotData> ShotData : ShotDataListBase)
	{
		...
		{
			// Get Camera
			if (ShotData->FileType=="Camera")
			{
				CameraFilePath = ShotData->LastestFilePath;;
				UE_LOG(LogTemp, Log, TEXT("Found Camera Match Naming Convention : %s"), *CameraFilePath);
			}

			// Get Alembic Cache
			else if (ShotData->FileType == "AlembicAnim" &&
				(ShotData->AssetStatus==EAssetStatus::Loaded || ShotData->AssetStatus == EAssetStatus::UpToDate))
			{
				AlembicAnimList.Add(ShotData);
			}
		}
		
	}

	// Start Bulidng Sequence
	FString &CameraPath = CameraFilePath;
	FString SequencePath = FPaths::Combine(TargetShotData->ContentAssetDirPath, FString("SEQ_") + TargetShotData->ShotName);

	UObject* CreateSequencer = AssetImporterUtility::CreateNewSequencerFromTemplate(
		TargetShotData->ContentAssetDirPath,
		FString("SEQ_") + TargetShotData->ShotName
	);

	if (CreateSequencer)
	{
		// Import Camera
		if (CameraFilePath.IsEmpty())
		{
			UE_LOG(LogTemp, Log, TEXT("- Not Found Camera Files"));
		}
		else
		{
			AssetImporterUtility::ImportCameraToSequencer(CreateSequencer, SequencePath, CameraFilePath);
			UE_LOG(LogTemp, Log, TEXT("- Import Camera Files : %s"), *CameraFilePath);
		}

		// Import Alembic
		if (AlembicAnimList.IsEmpty())
		{
			UE_LOG(LogTemp, Log, TEXT("- Not Found Alembic Anim Files"));

		}
		else
		{
			for (TSharedPtr<FShotData> ShotData : AlembicAnimList)
			{
				UE_LOG(LogTemp, Log, TEXT("- Alembic Camera Files : %s"), *(ShotData->CurrentImportPath));
				AssetImporterUtility::AddAlembicToSequencer(SequencePath,
					ShotData->ContentAssetFilePath,
					FPaths::GetBaseFilename(ShotData->LastestFilePath));
	...
	{
		UE_LOG(LogTemp, Log, TEXT("# Failed to Quick Build New Sequencer"));
	}

}

void SUkoreAssetImporterWidgetTab::OnReimportCameraOnlyClicked()
{
	TArray<TSharedPtr<FShotData>> Selected = SUkoreAssetImporterWidgetTab::GetSelectedItemListView();
	...
	TSharedPtr<FShotData> TargetReimport = Selected[0];
	
	if (TargetReimport->FileType != "Camera")
	{
		return;
	}
	
	FString SequencerPath = FPaths::Combine(TargetReimport->ContentAssetDirPath, FString("SEQ_") + TargetReimport->ShotName);

	UObject* LoadedAsset = StaticLoadObject(UObject::StaticClass(), nullptr, *SequencerPath);
	bool bImportCamera = AssetImporterUtility::ImportCameraToSequencer(LoadedAsset, SequencerPath, TargetReimport->LastestFilePath);

	if (bImportCamera)
	{
		UE_LOG(LogTemp,Log, TEXT("# Remport Camera Success!"));
	}
	else {
		UE_LOG(LogTemp, Log, TEXT("# Remport Camera Failed!"));
	}
}
