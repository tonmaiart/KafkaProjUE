...
namespace AssetImporterUtility{
	...
	{
		// Get Level Sequence
		UE_LOG(LogTemp, Log, TEXT("# AddAlembicToSequencer #"))
		...
		if (!LevelSequence)
		{
			UE_LOG(LogTemp, Log, TEXT("- Level Sequence Invalid "))
		...
		if (!World)
		{
			UE_LOG(LogTemp, Log, TEXT("- Get World Invalid "))
		...
		if (!MovieScene)
		{
			UE_LOG(LogTemp, Log, TEXT("- Get Movie Scene Invalid "))
		...
		if (!GeoCache)
		{
			UE_LOG(LogTemp, Log, TEXT("- Get GeometryCache Invalid "))
		...
		UMovieSceneGeometryCacheTrack* GeoCacheTrack = MovieScene->AddTrack<UMovieSceneGeometryCacheTrack>(ActorGuid);
		...
		if (GeoCacheSection)
		{
			...
			FMovieSceneGeometryCacheParams& Params = GeoCacheSection->Params;
			Params.GeometryCacheAsset = GeoCache;
			Params.StartFrameOffset = 0;
			Params.PlayRate = 1.0f;
			Params.bReverse = false;

			GeoCacheTrack->AddSection(*GeoCacheSection);
		}

		LevelSequence->MarkPackageDirty();

		UE_LOG(LogTemp, Log, TEXT("- Alembic Successful Added : %s"), *ActorLabel);
